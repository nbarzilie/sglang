...
srun \
  ...
  -p interactive \
  --gpus-per-node=8 \
  --container-image="$MY/sqshs/sglang-fresh.sqsh" \
  --container-save="$MY/sqshs/sglang_nixl.sqsh" \
  --container-writable \
...
echo
echo "NIXL image filesystem is ready to inspect. Exit this shell when you want Pyxis to save:"
echo "  $MY/sqshs/sglang_nixl.sqsh"
exec bash -i
'
